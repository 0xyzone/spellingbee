...
<nav x-data="{ 
        ...
        <div class="hidden lg:flex items-center gap-12 overflow-visible">
            <ul class="flex items-center gap-10 list-none overflow-visible">
                ...
                <li class="overflow-visible h-full">
                    <a href="{{ $link['url'] }}" @if($link['external']) target="_blank" @endif class="font-black text-[18px] uppercase  transition-all duration-300 hover:text-amber-500 relative group overflow-visible" :class="atTop ? 'text-slate-900' : 'text-slate-700'">
                        <p class="flex items-center w-full gap-2">
                            {{ $link['name'] }}
                            @if ($link['name'] == 'Evention Master')
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 48 48" class="w-6 h-6 inline group-hover:fill-amber-500 transition-all duration-300 overflow-visible">
    ...
    <template x-teleport="body">
        <div x-show="paymentModalOpen" x-data="{ 
            ...
            <div x-show="paymentModalOpen" x-transition:enter="transition cubic-bezier(0.34, 1.56, 0.64, 1) duration-600" x-transition:enter-start="opacity-0 translate-y-full lg:scale-90" x-transition:enter-end="opacity-100 translate-y-0 lg:scale-100" x-transition:leave="transition ease-in duration-400" x-transition:leave-start="opacity-100 translate-y-0 lg:scale-100" x-transition:leave-end="opacity-0 translate-y-full lg:scale-95" {{-- Handle touch events on the whole panel --}} @touchstart="handleTouchStart($event)" @touchmove="handleTouchMove($event)" @touchend="handleTouchEnd()" :style="`transform: translateY(${currentY}px); transition: ${isDragging ? 'none' : 'transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)'}`" class="relative bg-white w-full max-w-5xl h-[80vh] lg:h-auto lg:max-h-[90vh] rounded-t-[3rem] lg:rounded-[4rem] shadow-[0_-20px_100px_rgba(0,0,0,0.5)] z-10 flex flex-col lg:flex-row overflow-hidden">
